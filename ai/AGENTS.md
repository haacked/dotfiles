...
A skill listed in `ai/helpers/portable-skills.sh` loses that exception, because a cloud agent run unpacks it into a sandbox with no clone of this repo. Such a skill carries its helpers under its own `scripts/` and never writes a `~/.dotfiles/` path or a `/other-skill` reference in its `SKILL.md`. See the portable skills section of `ai/README.md`.
...
- Comments: default to none. The code already shows how it works, so comment only on what isn't obvious to a skilled reader: a constraint, a deliberate deviation, a gotcha, a workaround. Never narrate the code, restate a name or signature inline, or mark the end of a block. Write one fact per sentence, in plain grammar: active voice, finite verbs, no idioms, keep articles and relative pronouns. A comment with two facts is two sentences, not one joined by a comma or semicolon. A doc comment may also carry the contract a caller cannot see. The `comment-cleanup` skill enforces this over comments that already exist.
