...
A cloud agent run uploads one skill folder into a sandbox that has no clone of this repo, so a skill listed in `ai/helpers/portable-skills.sh` has to work with nothing but its own directory. Such a skill carries a copy of every helper it calls under its `scripts/` and a copy of every skill it hands a pass to under its `references/`, and its `SKILL.md` names no path outside that folder. The same `SKILL.md` also names no other skill with a leading slash, because PostHog Desktop reads `/other-skill` as a dependency and refuses the upload when that skill is a symlink the user did not select. A step that reads the bundled copy never raises that dependency, and it runs the same way whether or not the harness has the skill installed. `wait-for-pr-reviews` and `address-pr-reviews` are portable today.

Edit a helper or a vendored skill where the table says it comes from, then run `ai/bin/sync-portable-skills.sh` and commit the refreshed copies. Editing the copy instead loses the edit to the next sync, so `--check` names the source file to open. A copy and a hand-maintained file sit side by side and look alike, so `PORTABLE_SKILL_OWN_FILES` in the same table file lists the files a skill owns. Both lists write each path relative to the skill's own directory. Anything in the folder that neither list names is what a renamed or deleted row leaves behind: `--check` reports it, and a plain run deletes it. The reverse also fails: a file added to a vendored skill that no row copies is reported, because every other check starts from the table and would pass while the bundle fell behind. A source skill's own tests and its `__pycache__` are the two things that never travel.

A `references/<skill>/` copy keeps the source skill's folder shape, so a relative path written inside its `SKILL.md` resolves against the copy. The copy is verbatim, which is what lets `--check` compare it with `cmp`, so whatever the source skill names outside itself travels with it: a `~/.dotfiles/` path behind an argument this caller never passes, or a file such as `AGENTS.md` that the skill reads when the run has one. A skill worth vendoring says what to do when that file is absent, because the sandbox holds nothing but the folder. The lint below scans the folder's own root `SKILL.md` alone, which is also the one file PostHog Desktop reads for dependencies.
