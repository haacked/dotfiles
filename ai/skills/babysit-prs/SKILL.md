...
- If `mergeable` from the Step 2 fetch is `CONFLICTING`, say the PR can't enter the queue at all — Trunk never admits a conflicting PR, so a `waiting` submission is stuck rather than progressing. The row reads "stuck: conflicts with base — needs resolve + re-enqueue". Still hands-off here; an attended `/ci-monitor <number>` run takes its conflict-fix path from there.
...
- **Dispatch a conflict fix** when the gate denies with `pr_mergeable: "CONFLICTING"` in its verdict: a requeue is futile until the branch is rebased, so dispatch to `ci-monitor` exactly as above and let its 7b conflict-fix path finish the job. That path force-pushes (with lease), which is safe here only because the gate confirmed the drop this sweep.
