...
# Step 3 runs the bundled plain-writing lint. It is the only entry point outside
# scripts/, and a copy missing its lint script would otherwise surface as a failed
# prose pass mid-run in the sandbox.
# shellcheck disable=SC2016
assert_run 'lint prose through the bundled plain-writing copy' 0 \
  '[.warnings[].category]' '["jargon"]' \
  bash -c 'printf "%s" "$2" | python3 "$1/references/plain-writing/scripts/plain-writing-lint.py" --json' \
  bash "$sandbox/skill" 'We leverage the cache.'
