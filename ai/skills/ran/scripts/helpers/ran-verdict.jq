...
#       $commits [{sha,ts}] oldest first, $entries [{ts,step,sha}]
...
# A step counts as having run when its declared evidence says so: the log for a
# step a command records, the branch's own commits for one it does not.
| ($steps | map(
    . as $s
    | {key: $s.step,
       value: (if $s.evidence == "commits"
               then ($commits | length) > 0
               else ([$log[] | select(.step == $s.step)] | length) > 0
...
| ($order | map(
    . as $step
    | $rank[$step] as $myrank
    | ([$log[] | select(.step == $step)] | last) as $last
    | (if $decl[$step].evidence == "commits"
        ...
        then ($order[$myrank - 1]) as $prev
          # Due once the step before it has run; before that it is not yet its turn.
          | {status: (if ($ran[$prev] and ($decl[$step].optional | not))
