...
# fail-closed: `dropped` (a Trunk-initiated eviction), `waiting` (submitted,
# not yet taken), `unknown` (anything else, including a body matching both
# ways). This is a deliberate, bounded exception to the
...
# Each drop marker is a whole observed sentence rather than the shared
# "removed from the merge queue because" stem: that family also carries the
# human-action removals ("pushed to by @x", "closed by @x", "canceled by X"),
# all observed live, which must read unknown, not dropped - a person's call is
# never automated over. A newly observed Trunk-initiated wording gets its own
# marker and enum value here, judged in the requeue gate on its own semantics.
# The check-failure cause has two observed wordings: the required-check
...
def DROPPED_FAILED_TESTS_MARKER: "removed from the merge queue because it failed tests";
def DROPPED_UNMERGEABLE_MARKER: "removed from the merge queue because it was waiting to become mergeable for too long";
def DROPPED_RATE_LIMITED_MARKER: "removed from the merge queue because the GitHub API rate limit was exceeded";
...
# Only blocked earns a reason. check_failed outranks the no-tests-ran markers
# when a body carries several drop phrases: it is the stricter marker (the
# requeue gate demands an identifiable merge PR to triage for it).
| (if $state != "blocked" then {reason: null, marker: null}
   else ($comment.body // "") as $body
   | ($body | contains(WAITING_MARKER)) as $waiting
   | ($body | test(DROPPED_CHECK_FAILED_RE)
      or ($body | contains(DROPPED_FAILED_TESTS_MARKER))) as $check_failed
   | ($body | contains(DROPPED_TIMEOUT_MARKER)) as $timed_out
   | ($body | contains(DROPPED_UNMERGEABLE_MARKER)) as $unmergeable
   | ($body | contains(DROPPED_RATE_LIMITED_MARKER)) as $rate_limited
   | ($check_failed or $timed_out or $unmergeable or $rate_limited) as $any_drop
   | if $waiting and $any_drop then {reason: "unknown", marker: null}
     elif $check_failed then {reason: "dropped", marker: "check_failed"}
     elif $unmergeable then {reason: "dropped", marker: "unmergeable_timeout"}
     elif $rate_limited then {reason: "dropped", marker: "rate_limited"}
