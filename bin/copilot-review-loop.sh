...
# Get the latest Copilot review ID, or 0 if none exists.
get_latest_copilot_review_id() {
...
poll_for_review() {
  ...
  while [[ $elapsed -lt $POLL_TIMEOUT ]]; do
    ...
    local latest_id
    latest_id=$(get_latest_copilot_review_id 2>/dev/null || echo "0")

    if [[ "$latest_id" != "$before_id" && "$latest_id" != "0" ]]; then
      echo "$latest_id"
      return 0
    fi
...
main() {
  ...
  for ((round = 1; round <= MAX_ROUNDS; round++)); do
    ...
    # Capture the current latest Copilot review ID so we can detect a new one
    local before_id
    before_id=$(get_latest_copilot_review_id 2>/dev/null || echo "0")
    ...
    local review_id
    review_id=$(poll_for_review "$before_id") || {
      log_error "Copilot review did not appear within ${POLL_TIMEOUT}s"
      break
    }
    log_success "New Copilot review: ${review_id}"
